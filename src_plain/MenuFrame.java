...
public class MenuFrame extends JFrame {
	...
	private JLabel accountChip; // 右上角账号显示（点击弹出账号菜单）
	...
	private JPanel buildTopBar() {
		...
		accountChip = new JLabel(" ");
		accountChip.setForeground(new Color(0x9FE0B8));
		accountChip.setCursor(new java.awt.Cursor(java.awt.Cursor.HAND_CURSOR));
		accountChip.setBorder(BorderFactory.createEmptyBorder(4, 12, 4, 12));
		accountChip.addMouseListener(new MouseAdapter() {
			@Override public void mouseClicked(MouseEvent e) { showAccountDialog(); }
		});
		JPanel right = new JPanel();
		right.setOpaque(false);
		right.add(accountChip);
	...
	private void showPage(String card, String title) {
		...
		accountChip.setText(LoginPanel.lastLoginUser == null ? "未登录" : "👤 " + LoginPanel.lastLoginUser);
		setTitle(title);
		// 按页面调整窗口尺寸并居中
	...
	/** 账号菜单：显示当前用户，提供退出登录 / 修改密码 */
	private void showAccountDialog() {
		String user = LoginPanel.lastLoginUser;
		if (user == null) {
			JOptionPane.showMessageDialog(this, "当前未登录", "账号", JOptionPane.INFORMATION_MESSAGE);
			return;
		}
		String[] opts = { "退出登录", "修改密码", "取消" };
		int pick = JOptionPane.showOptionDialog(this, "当前账号：" + user, "账号",
				JOptionPane.DEFAULT_OPTION, JOptionPane.PLAIN_MESSAGE, null, opts, opts[2]);
		if (pick == 0) { // 退出登录：回菜单，清登录态
			if (PAGE_GAME.equals(page) && currentGame != null) currentGame.shutdown();
			LoginPanel.lastLoginUser = null;
			backToMenu();
		} else if (pick == 1) {
	...
	/** 修改密码：旧密码 + 新密码两次输入，PlayerStore.changePassword 落盘 */
	private void promptChangePassword(String user) {
		JPasswordField oldF = new JPasswordField();
		JPasswordField newF = new JPasswordField();
		JPasswordField newF2 = new JPasswordField();
		Object[] msg = { "旧密码:", oldF, "新密码:", newF, "确认新密码:", newF2 };
		int ok = JOptionPane.showConfirmDialog(this, msg, "修改密码 · " + user,
				JOptionPane.OK_CANCEL_OPTION, JOptionPane.PLAIN_MESSAGE);
		if (ok != JOptionPane.OK_OPTION) return;
		String oldPw = new String(oldF.getPassword());
		String p1 = new String(newF.getPassword());
		String p2 = new String(newF2.getPassword());
		if (!p1.equals(p2)) { JOptionPane.showMessageDialog(this, "两次输入的新密码不一致", "错误", JOptionPane.ERROR_MESSAGE); return; }
		if (PlayerStore.passProblem(p1.toCharArray()) != null) { JOptionPane.showMessageDialog(this, PlayerStore.passProblem(p1.toCharArray()), "错误", JOptionPane.ERROR_MESSAGE); return; }
		int rc = PlayerStore.changePassword(user, oldPw, p1);
		if (rc == 0) JOptionPane.showMessageDialog(this, "密码修改成功", "完成", JOptionPane.INFORMATION_MESSAGE);
		else if (rc == -1) JOptionPane.showMessageDialog(this, "旧密码错误", "错误", JOptionPane.ERROR_MESSAGE);
		else JOptionPane.showMessageDialog(this, "修改失败（错误码 " + rc + "）", "错误", JOptionPane.ERROR_MESSAGE);
	}
